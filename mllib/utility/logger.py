...
class Logger:
    ...
    __slack_webhook_url: str = None
    __slack: slackweb.Slack = None
    ...
    def set_webhook(webhook_url: str) -> None:
        ...
        """
        Logger.__slack_webhook_url = webhook_url
        Logger.__slack = slackweb.Slack(webhook_url)
    ...
    def __log_to_slack(self, loglevel: int, message: str) -> None:
    ...
    def debug(self, message: str, should_log_slack: bool = True):
        message = Logger.__make_message(message)
        self.logger.debug(message)
        if should_log_slack:
            self.__log_to_slack(logging.DEBUG, "DEBUG " + message)

    def info(self, message: str, should_log_slack: bool = True):
        message = Logger.__make_message(message)
        self.logger.info(message)
        if should_log_slack:
            self.__log_to_slack(logging.INFO, "INFO " + message)

    def warning(self, message: str, should_log_slack: bool = True):
        message = Logger.__make_message(message)
        self.logger.warning(message)
        if should_log_slack:
            self.__log_to_slack(logging.WARNING, "WARNING " + message)

    def error(self, message: str, should_log_slack: bool = True):
        message = Logger.__make_message(message)
        self.logger.error(message)
        if should_log_slack:
            self.__log_to_slack(logging.ERROR, "ERROR " + message)

    def critical(self, message: str, should_log_slack: bool = True):
        message = Logger.__make_message(message)
        self.logger.critical(message)
        if should_log_slack:
            self.__log_to_slack(logging.CRITICAL, "CRITICAL " + message)
